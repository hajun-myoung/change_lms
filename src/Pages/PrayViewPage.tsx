...
export default function PrayViewPage() {
  const { id } = useParams<{ id: string }>();
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [announcement, setAnnouncements] = useState<Pray>();
  ...
  useEffect(() => {
    const fetchAssignments = async () => {
      ...
      const prayRef = doc(db, "pray_board", id);
      const praySnap = await getDoc(prayRef);
      console.log(praySnap);

      if (praySnap.exists()) {
        setAnnouncements({
          ...(praySnap.data() as Pray),
          id: praySnap.id,
        ...
        <Box sx={{ display: "flex", alignItems: "center" }}>
          ...
          >
            ...
            <Typography variant="course_info">
              ...
              {
                users.filter(
                  (user) => user.student_id == announcement?.author_id
                )[0]?.name
              }
